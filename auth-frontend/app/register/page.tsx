...
export default function RegisterPage() {
    ...
    import('animejs').then(({ animate, stagger, createTimeline }) => {
      if (particlesRef.current) {
        ...
        anim = animate('.particle', {
          ...
          opacity: [0.05, 0.5], scale: [0.7, 1.3],
          duration: () => Math.random()*3500+2000,
          delay: stagger(100), loop: true, direction: 'alternate', easing: 'easeInOutSine',
        }) as { pause: () => void };
      }
  ...
  const fields = [
    { label: 'Full Name',       type: 'text',     placeholder: 'John Doe',          value: name,     set: setName },
    { label: 'Email Address',   type: 'email',    placeholder: 'you@example.com',   value: email,    set: setEmail },
    { label: 'Password',        type: 'password', placeholder: '•••••••• (min 6)',  value: password, set: setPassword },
  ];
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', padding:'1.5rem', position:'relative', overflow:'hidden' }}>
      ...
      <div style={{ position:'absolute', inset:0, pointerEvents:'none', overflow:'hidden' }}>
        <div style={{ position:'absolute', top:'-10%', right:'-10%', width:'480px', height:'480px', background:'radial-gradient(circle, rgba(56,176,0,0.18) 0%, transparent 65%)', borderRadius:'50%' }} />
        <div style={{ position:'absolute', bottom:'-20%', left:'-5%', width:'560px', height:'560px', background:'radial-gradient(circle, rgba(112,224,0,0.12) 0%, transparent 65%)', borderRadius:'50%' }} />
      </div>
      ...
      <div ref={cardRef} className="card" style={{ width:'100%', maxWidth:'420px', padding:'2.5rem', position:'relative', zIndex:10, opacity:0 }}>
        ...
        <div style={{ textAlign:'center', marginBottom:'2rem' }}>
          <div style={{ display:'inline-flex', alignItems:'center', justifyContent:'center', width:'60px', height:'60px', borderRadius:'16px', background:'linear-gradient(135deg, #38b000, #ccff33)', marginBottom:'1.25rem', boxShadow:'0 4px 20px rgba(112,224,0,0.4)' }}>
            <svg width="30" height="30" viewBox="0 0 24 24" fill="none">
              ...
              <path d="M16 3.13a4 4 0 0 1 0 7.75" stroke="#002a14" strokeWidth="2" strokeLinecap="round"/>
            </svg>
          </div>
          <h1 className="glow-text" style={{ fontSize:'1.8rem', fontWeight:800, lineHeight:1.2 }}>Create Account</h1>
          <p style={{ color:'rgba(158,240,26,0.55)', fontSize:'0.875rem', marginTop:'0.4rem' }}>Fill in your details to get started</p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:'1.1rem' }}>
          {fields.map(({ label, type, placeholder, value, set }) => (
            <div key={label} className="form-field" style={{ opacity:0 }}>
              ...
              <input type={type} placeholder={placeholder} value={value} onChange={e => set(e.target.value)} className="input-field" required />
            </div>
          ))}
        ...
        <p className="form-field" style={{ textAlign:'center', fontSize:'0.875rem', color:'rgba(158,240,26,0.45)', opacity:0 }}>
          ...
          <Link href="/login" style={{ color:'var(--chartreuse)', fontWeight:700, textDecoration:'none' }}>
            Sign in
          </Link>
        </p>
      </div>
